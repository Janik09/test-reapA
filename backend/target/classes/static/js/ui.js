...
export function renderLogin(root, state, actions) {
  root.innerHTML = `
    <section class="grid grid--2">
      ...
      <div class="card">
        <h3 class="section-title">Willkommen zurÃ¼ck</h3>
  ...
  root.innerHTML = `
    <section class="grid grid--2">
      <div class="card">
        ...
        <p>Plane deinen Restaurantbesuch entspannt: Reserviere einen Tisch, wÃ¤hle Gerichte aus und bezahle online â€“ alles an einem Ort.</p>
        <div class="grid">
  ...
  root.innerHTML = `
    <section class="grid grid--2">
      ...
      <div class="card" id="reservationResult">
        <h3 class="section-title">BestÃ¤tigung</h3>
        ${reservationGallery()}
  ...
  root.innerHTML = `
    <section class="grid grid--2">
      <div class="card">
        ...
        ${state.cart.length === 0 ? '<p>Dein Warenkorb ist leer.</p>' : state.cart.map((item) => `
          <div class="cart-item">
            <div>
              ...
              <small class="text-muted">${item.price.toFixed(2)} â‚¬</small>
            </div>
        ...
        `).join('')}
        <div class="order-total"><strong>Gesamt: ${total.toFixed(2)} â‚¬</strong></div>
      </div>
...
export function renderChef(root, state, actions) {
  ...
  root.innerHTML = `
    <section class="card">
      <h2 class="section-title">KÃ¼che</h2>
      <p class="text-muted">Bezahlte Bestellungen in Reihenfolge der nÃ¤chsten Reservierung (First come, first serve).</p>
    </section>
...
export function renderContact(root) {
  root.innerHTML = `
    <section class="card">
      ...
      <p>Rufe uns an oder schreibe uns eine Nachricht â€“ wir freuen uns auf dich!</p>
      <p><strong>Telefon:</strong> +49 30 1234567</p>
      <p><strong>E-Mail:</strong> hallo@restaurantapp.de</p>
      <p><strong>Adresse:</strong> MusterstraÃŸe 10, 10115 Berlin</p>
      <p>Ã–ffnungszeiten: Mo-So 12:00â€“22:30 Uhr</p>
    </section>
  `;
}
...
function reservationSummary(reservation) {
  return `
    <div class="summary-item">
      <strong>#${reservation.id}</strong> â€“ ${reservation.customerName}<br />
  ...
  return `
    <div class="reservation-media">
      <div class="reservation-media__item">
        ...
        <div class="menu-card__fallback hidden">Bild nicht verfÃ¼gbar</div>
...
function orderSummary(order, includePay = false) {
  return `
    <div class="summary-item">
      <strong>#${order.id}</strong> â€“ ${order.customerName}<br />
      <small>Status: ${order.status} | Gesamt: ${order.total.toFixed(2)} â‚¬</small><br />
  ...
  return `
    <article class="card">
      <div class="chef-order__header">
        ...
        <span class="badge price-badge">${total.toFixed(2)} â‚¬</span>
      </div>
